...
function ChatViewInner({ conversationId }: ChatViewProps): React.ReactElement {
  ...
  const useFluidWidth = !sidebarCollapsed && !promptSidebarOpen
  ...
  /** 加载全部历史消息（向上滚动时触发） */
  const handleLoadMore = React.useCallback(async (): Promise<LoadMoreResult> => {
    const currentMessages = messages
    const allMessages = await window.electronAPI.getConversationMessages(conversationId)
    let prependedMessages: ChatMessage[] = []

    if (currentMessages.length === 0) {
      prependedMessages = allMessages
    } else {
      const firstCurrentId = currentMessages[0]?.id
      if (firstCurrentId) {
        const firstCurrentIndex = allMessages.findIndex((message) => message.id === firstCurrentId)
        if (firstCurrentIndex > 0) {
          prependedMessages = allMessages.slice(0, firstCurrentIndex)
        }
      }
    }
