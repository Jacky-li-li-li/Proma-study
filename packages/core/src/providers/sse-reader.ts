...
import type { ProviderAdapter, ProviderRequest, StreamEventCallback, StreamUsage, ToolCall } from './types.ts'
...
function mergeUsage(base: StreamUsage | undefined, incoming: StreamUsage): StreamUsage {
  return {
...
export async function streamSSE(options: StreamSSEOptions): Promise<StreamSSEResult> {
  ...
  return { content, reasoning, toolCalls, stopReason, usage }
}
