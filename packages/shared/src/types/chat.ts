...
export interface ChatTokenUsage {
  /** 输入 token（包含 cache read / cache creation） */
  inputTokens?: number
  /** 输出 token */
  outputTokens?: number
  /** 缓存读取 token */
...
/** 统计来源范围 */
export type UsageScope = 'all' | 'chat' | 'agent'
...
/** 单个范围的统计结果 */
export interface UsageScopeStats {
  /** 全历史按天聚合数据（升序） */
  daily: UsageDayBucket[]
  /** 全历史总 token */
  allTimeTotalTokens: number
  /** 全历史按模型聚合 */
  allTimeByModel: Record<string, number>
}
...
export interface ModelUsageStats {
  ...
  generatedAt: number
  /** 三种来源范围的统计 */
  scopes: Record<UsageScope, UsageScopeStats>
}
...
export const CHAT_IPC_CHANNELS = {
  ...
  /** 获取模型用量统计（全历史） */
  GET_MODEL_USAGE_STATS: 'chat:get-model-usage-stats',
  /** 更新对话标题 */
  ...
  READ_LOCAL_IMAGE: 'chat:read-local-image',
